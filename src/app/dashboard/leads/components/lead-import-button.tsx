...
export default function LeadImportButton({ queryKey = "admin-leads" }: { queryKey?: string }) {
    ...
    const fallbackPrograms: Program[] = [
        { _id: "69e8c025afaf0d3fb90233d4", name: "NLP Master Trainer Program", price: 0 },
        {
        ...
        { _id: "69d8a8ed06f01d73ae725722", name: "NLP Master Practitioner Program", price: 0 },
        { _id: "69d88bcd3b3f401bb2e711bc", name: "NLP Practitioner Program", price: 0 },
    ];
    ...
    const { data: programs = fallbackPrograms } = useQuery({
        ...
        queryFn: async () => {
            try {
                const res = await getNamesPrograms();
                const list = res?.data;
                if (!Array.isArray(list) || list.length === 0) {
                    console.warn("Programs API returned empty — using fallback list");
                    return fallbackPrograms;
                }
                return list;
            } catch (err) {
                console.error("Programs API failed — using fallback list", err);
                return fallbackPrograms;
            }
        },
    });
